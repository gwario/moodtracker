...
import at.ameise.moodtracker.app.activity.SimpleImportExportActivity;
...
public class MoodHistoryFragment extends Fragment implements LoaderManager.LoaderCallbacks<Cursor> {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        applicationPreferences = getActivity().getSharedPreferences(PreferenceConstant.APPLICATION_PREFERENCES, Context.MODE_PRIVATE);
        ...
        if (getArguments() != null) {
            ...
            mDisplayingLoader = getArguments().getInt(ARG_INITIAL_LOADER, LoaderId.MOOD_HISTORY_PER_QUARTER_DAY_LOADER);

        } else {

            mDisplayingLoader = LoaderId.MOOD_HISTORY_PER_QUARTER_DAY_LOADER;
        }
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        //init loaders
        getLoaderManager().initLoader(LoaderId.MOOD_HISTORY_PER_QUARTER_DAY_LOADER, null, this);
        getLoaderManager().initLoader(LoaderId.MOOD_HISTORY_PER_DAY_LOADER, null, this);
        getLoaderManager().initLoader(LoaderId.MOOD_HISTORY_PER_WEEK_LOADER, null, this);
        getLoaderManager().initLoader(LoaderId.MOOD_HISTORY_PER_MONTH_LOADER, null, this);
    ...
    public boolean onOptionsItemSelected(MenuItem selectedItem) {
        ...
        switch (itemId) {
            case R.id.menu_item_history_day_values:
                if(mDisplayingLoader != LoaderId.MOOD_HISTORY_PER_QUARTER_DAY_LOADER) {
                    mDisplayingLoader = LoaderId.MOOD_HISTORY_PER_QUARTER_DAY_LOADER;
            ...
            case R.id.menu_item_history_daily_avg:
                if(mDisplayingLoader != LoaderId.MOOD_HISTORY_PER_DAY_LOADER) {
                    mDisplayingLoader = LoaderId.MOOD_HISTORY_PER_DAY_LOADER;
                    repopulateChart(perDayCursor, dailyDateFormat);
                }
                return true;

            case R.id.menu_item_history_weekly_avg:
                if(mDisplayingLoader != LoaderId.MOOD_HISTORY_PER_WEEK_LOADER) {
                    mDisplayingLoader = LoaderId.MOOD_HISTORY_PER_WEEK_LOADER;
                    repopulateChart(perWeekCursor, weeklyDateFormat);
                }
                return true;

            case R.id.menu_item_history_month_avg:
                if(mDisplayingLoader != LoaderId.MOOD_HISTORY_PER_MONTH_LOADER) {
                    mDisplayingLoader = LoaderId.MOOD_HISTORY_PER_MONTH_LOADER;
                    repopulateChart(perMonthCursor, monthlyDateFormat);
                }
                return true;

            case R.id.menu_item_syncNow:
                long lastManualSyncTimestamp = applicationPreferences.getLong(PreferenceConstant.KEY_LAST_MANUAL_SYNC_TIMESTAMP, -1);
                ...
                if(!hasSyncedManually
                    ...
                    applicationPreferences.edit().putLong(PreferenceConstant.KEY_LAST_MANUAL_SYNC_TIMESTAMP, DateTime.now().getMillis()).commit();
                } else {
                    Logger.info(TagConstant.SYNCHRONIZATION, "Manual sync denied.");
                }
                return true;
            ...
            case R.id.menu_item_share:
                if(perQuarterDayCursor != null) {
                    if (perQuarterDayCursor.moveToLast())
                        ...
                        ToastUtil.showNoMoodsYetText(getActivity());
                } else {
                    ToastUtil.showMoodsNotYetLoadedText(getActivity());
                }
                return true;
    ...
    private void repopulateChart(Cursor data, SimpleDateFormat dateFormat) {
        ...
        if (data.moveToFirst()) {
            ...
            do {
                ...
                Logger.verbose(TagConstant.MOOD_HISTORY, mood.toString());
        ...
        Logger.info(TagConstant.MOOD_HISTORY, "Found " + data.getCount() + " moods");
...
//        switch(cursorId) {
//
//            case LoaderId.MOOD_HISTORY_PER_DAY_LOADER:
//                break;
//
//            case LoaderId.MOOD_HISTORY_PER_WEEK_LOADER:
//                break;
//
//            case LoaderId.MOOD_HISTORY_PER_MONTH_LOADER:
//                break;
//
//            case LoaderId.MOOD_HISTORY_PER_QUARTER_DAY_LOADER:
//            default:
//
//                break;
//        }
        ...
        lineChart.getAxisLeft().setValueFormatter(new DefaultYAxisValueFormatter());
        lineChart.getAxisRight().setAxisMinValue(0.5f);
        lineChart.getAxisRight().setAxisMaxValue(7.5f);
        lineChart.getAxisRight().setValueFormatter(new DefaultYAxisValueFormatter());
...
//        lineChart.setMaxVisibleValueCount(18);//hide value labels after 18 values in viewport
    ...
    public void onLoadFinished(Loader<Cursor> loader, Cursor data) {
        ...
        if(loader.getId() == LoaderId.MOOD_HISTORY_PER_QUARTER_DAY_LOADER) {

            perQuarterDayCursor = data;
            if(loader.getId() == mDisplayingLoader)
                repopulateChart(data, quarterDailyDateFormat);

        } else if(loader.getId() == LoaderId.MOOD_HISTORY_PER_DAY_LOADER) {

            perDayCursor = data;
            if(loader.getId() == mDisplayingLoader)
                repopulateChart(data, dailyDateFormat);

        } else if(loader.getId() == LoaderId.MOOD_HISTORY_PER_WEEK_LOADER) {

            perWeekCursor = data;
            if(loader.getId() == mDisplayingLoader)
                repopulateChart(data, weeklyDateFormat);

        } else if(loader.getId() == LoaderId.MOOD_HISTORY_PER_MONTH_LOADER) {
    ...
    public void onLoaderReset(Loader<Cursor> loader) {
        ...
        if(loader.getId() == LoaderId.MOOD_HISTORY_PER_DAY_LOADER)
            perDayCursor = null;
        if(loader.getId() == LoaderId.MOOD_HISTORY_PER_WEEK_LOADER)
            perWeekCursor = null;
        if(loader.getId() == LoaderId.MOOD_HISTORY_PER_MONTH_LOADER)
            perMonthCursor = null;
    }
